...
- For background mode, switch to Terminal or another app. Leave WoW running with
  its window available, not minimised. Do not bring it to the foreground during
  the run: background mode deliberately stops when WoW becomes the foreground app.
...
That mode activates WoW and requires it to remain foreground. Report its mode
...
| --- | --- |
| `status` | `completed` |
| `mode` | The mode being claimed, normally `background` |
| `autonomous_seconds` | At least 300 |
| Pre-go log | Exactly one `pre_go_pass`; weapon switch when testing that transition |
| Every cycle | `loot_collected`, one cast and one retrieval click |
| `verified_loot_cycles` | Equal to the number of cycles, with no failed casts excluded |
| `perfect_run` | `true` |
| `provider_calls` | `0` |
| Manual intervention during the interval | None |

A successful process exit or `status: completed` alone is insufficient.
`loot_collected` means the fish label was observed, selected and the loot window
then cleared; it is not a direct inventory query. Review `after.jpg` and
`collected.jpg` alongside logs. Record all failed attempts separately.
...
| `stopped_focus_or_geometry` | Foreground mode requirement and unchanged window bounds |
...
| `loot_item_unconfirmed` / `loot_not_cleared` | Loot label, available bag space and any overlay |

Unknown or uncertain loot stops the runner for review. Other failed cycles may
...
Capture is capped at 10 Hz in a small region, retaining the latest frame rather
