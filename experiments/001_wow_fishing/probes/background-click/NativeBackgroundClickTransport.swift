...
// Local modification: preserve the audited routing and support explicit right-button pairs.
// Upstream source/commit and MIT attribution are recorded in this directory's README/LICENSE.
...
final class NativeBackgroundClickTransport {
    func move(target: RoutedClickTarget, point: CGPoint) throws {
        _ = try prepareTargetWindowForInput(target: target)
        guard let event = makeNSEventBackedCGEvent(type: .mouseMoved, point: point,
            windowLocal: windowLocalTopLeft(point: point, target: target), clickState: 0, target: target) else {
            throw ClickTransportError.transportFailed("Could not prepare target-local move")
        }
        event.timestamp = clock_gettime_nsec_np(CLOCK_UPTIME_RAW)
        NativeClickSymbols.slEventPostToPid(target.pid, event)
    }
    ...
    private func makeEventSequence(request: NativeBackgroundClickDispatchRequest) -> [CGEvent] {
        ...
        for clickState in (1...request.clickCount).map(Int64.init) {
            appendEvent(
                type: request.mouseButton == .right ? .rightMouseDown : .leftMouseDown,
                point: request.eventTapPointTopLeft,
                windowLocal: targetWindowLocal,
            ...
            appendEvent(
                type: request.mouseButton == .right ? .rightMouseUp : .leftMouseUp,
                point: request.eventTapPointTopLeft,
                windowLocal: targetWindowLocal,
                clickState: clickState,
                target: request.target,
    ...
    private func stamp(
        ...
        event.setIntegerValueField(.mouseEventButtonNumber, value: type == .rightMouseDown || type == .rightMouseUp ? 1 : 0)
