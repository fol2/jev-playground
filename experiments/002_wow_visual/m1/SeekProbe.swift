...
//                          a target; no input
//   --execute --keys K --look FRAME --box X,Y,W,H [--stop-growth G]
...
/// M0's shell plus the tracker (live) or the simulated world (dry-run).
final class SeekShell: ShellDriver, SeekDriver {
    ...
    init(log: Log, live: LiveTarget?, tracker: Tracker?, world: SimWorld?) {
    ...
    func sight(expect: Double) -> Sighting? {
        ...
        guard let match = tracker.sight(image, expect: expect) else { return nil }
        shown = match
    ...
    /// The newest frame with the tracker's latest match drawn on it.
    override func snapshot(_ name: String) {
        guard let live, let tracker, let image = live.feed.latest else { return }
...
@MainActor
func seekExecute(_ command: SeekCommand, profile: KeyProfile, box: Box, lookPath: String) async throws -> Int32 {
    let session = try await wowSession(input: true)
    ...
    guard let source = CGImageSourceCreateWithURL(URL(fileURLWithPath: lookPath) as CFURL, nil),
          let designationFrame = CGImageSourceCreateImageAtIndex(source, 0, nil) else { throw ProbeError("cannot read the look frame") }
    guard designationFrame.width == width, designationFrame.height == height else {
        throw ProbeError("look frame is \(designationFrame.width)x\(designationFrame.height) but live capture is \(width)x\(height); look again")
    }
    let factor = Double(trackWidth) / Double(width)
    func track(_ value: Int) -> Int { Int((Double(value) * factor).rounded()) }
    let trackBox = Box(x: track(box.x), y: track(box.y), width: track(box.width), height: track(box.height))
    guard let template = cropped(grey(designationFrame, width: trackWidth), trackBox) else { throw ProbeError("box lies outside the look frame") }

    let run = try runDirectory("m1")
    let log = try Log(file: run.url.appendingPathComponent("events.jsonl"))
    if let marked = annotated(designationFrame, box: CGRect(x: box.x, y: box.y, width: box.width, height: box.height)) {
    ...
    let first = await firstFrame(feed)
    let found = first.flatMap { frame -> Match? in
        let image = grey(frame.image, width: trackWidth)
        return locate(template, in: image, scales: [0.95, 1, 1.05], near: designated, reach: (Double(image.width), Double(image.height)))
    }
    let confirmed = found.map { $0.score >= 0.8 && $0.score - $0.runnerUp >= 0.1
        && hypot($0.x - designated.x, $0.y - designated.y) <= 6 } ?? false
    let tracker = found.map { Tracker(template: template, start: $0, minScore: config.minScore) }
    ...
    let driver = SeekShell(log: log, live: target, tracker: tracker, world: nil)
    let lease = InputLease(profile: profile, sink: PidKeySink(pid: session.app.processIdentifier), clock: hostNow,
                           emit: driver.emit, budget: SeekLimits.budget)
    let signals = trapSignals(lease, log)
    var manifest: [String: Any] = [
        "schema": "m1-run/v1", "run_id": run.id, "mode": "execute", "started_utc": ISO8601DateFormatter().string(from: Date()),
        ...
        "config": ["turn_rate": config.turnRate, "dead_s": config.dead, "gain": config.gain, "tolerance": config.tolerance,
                   "approach_tolerance": config.approachTolerance, "stop_growth": config.growth, "min_score": config.minScore,
    ...
    if confirmed {
        ...
        driver.emit("designation_refused", ["found": orNull(manifest["designation"]), "keys_sent": 0])
        result = SeekResult(outcome: "HOLD_DESIGNATION_NOT_CONFIRMED", pulses: [], facing: nil, final: nil, frameCounts: [:])
    }
    try? await stream.stopCapture()
    withExtendedLifetime(signals) {}
    ...
    manifest["meaning"] = "dispatch, tracked image position and apparent growth only; facing, approach and identity need labels.json"
    let labels: [String: Any] = [
        "instructions": "Fill from designation.jpg, the pNN JPEGs and what was seen live. null means not assessed.",
...
struct M1Seek {
    static func main() async {
        ...
        do {
            ...
            case .execute: exit(try await seekExecute(command, profile: command.profile!, box: command.box!, lookPath: command.look!))
            }
        } catch {
            fputs("HOLD: \(error)\n", stderr)
            exit(2)
