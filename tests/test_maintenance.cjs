...
async function scenario(change = {}, issues = [], current = sha) {
  const writes = [];
  const run = {id: 10, workflow_id: 5, event: 'push', head_branch: 'main',
    ...
    head_sha: sha, conclusion: 'failure', ...change};
  const github = {rest: {
    actions: {getWorkflow: async () => ({data: {id: 5, path: '.github/workflows/ai-sdlc.yml'}})},
  ...
  await execute(github, {repo: {owner: 'fol2', repo: 'jev-playground'}, payload: {workflow_run: run}});
  return writes;
}
